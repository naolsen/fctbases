...
arma::vec eval_coefs(double x) {
  ...
  if (x < x_min || x > x_max) {
    ...
    arma::vec::const_iterator it = --upper_bound(tknots.begin(), tknots.end(), x);
    
    int dist = distance(tknots.begin(), it);
    ret(dist) = 1;
    
    for (int j = 2; j <= order; j++) {
      ret(dist-j+1) =  (tknots(dist+1) - x)/( tknots(dist+1) - tknots(dist-j+2))* ret(dist-j+2);
      if (j > 2) for (int k = dist-j+2; k < dist; k++) {
        ret(k) =(x- tknots(k))/( tknots(k+j-1) - tknots(k))* ret(k) +
          ( tknots(k+j) - x)/( tknots(k+j) - tknots(k+1))* ret(k+1)  ;
      }
      ret(dist) =(x- *it)/(tknots(dist+j-1) - *it)* ret(dist);
    }
  }
  return ret;
}
